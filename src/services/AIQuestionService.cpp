...
AIQuestionResult AIQuestionService::generateQuestions(const AIQuestionRequest& request) {
    ...
    if (shouldUseMock()) {
        ...
        result.message = "Dang dung du lieu mau vi chua cau hinh AI that.";
        return result;
    }
    ...
    if (!callRealApi(request, result.prompt, response)) {
        ...
        result.message = lastError.empty()
                             ? "Khong goi duoc dich vu AI. Vui long kiem tra cau hinh."
                             : lastError;
        return result;
    }
    ...
    result.success = true;
    result.message = "Da sinh cau hoi bang AI.";
    return result;
}
...
bool AIQuestionService::parseGeneratedQuestions(const string& responseText,
    ...
    try {
        ...
        errorMessage = string("Khong doc duoc JSON AI tra ve: ") + error.what();
        return false;
    }
...
bool AIQuestionService::validateDraft(const GeneratedQuestionDraft& draft,
    ...
    if (draft.content.empty()) {
        errorMessage = "Thieu noi dung cau hoi.";
        return false;
    }
    if (draft.subject.empty()) {
        errorMessage = "Thieu mon hoc.";
        return false;
    }
    if (draft.difficulty.empty()) {
        errorMessage = "Thieu do kho.";
        return false;
    }
    for (int i = 0; i < 4; i++) {
        if (draft.options[i].empty()) {
            errorMessage = "Thieu dap an " + string(1, (char)('A' + i)) + ".";
            return false;
        }
    }
    if (draft.correctAnswer < 'A' || draft.correctAnswer > 'D') {
        errorMessage = "Dap an dung phai la A, B, C hoac D.";
        return false;
    }
    return true;
}
...
vector<GeneratedQuestionDraft> AIQuestionService::mockQuestions(const AIQuestionRequest& request) const {
    ...
    for (int i = 0; i < count; i++) {
        ...
        draft.subject = request.subject.empty() ? "Mon hoc" : request.subject;
        draft.difficulty = request.difficulty.empty() ? "Trung binh" : request.difficulty;
        draft.content = "Cau " + to_string(i + 1) + ": Noi dung nao phu hop voi chu de \"" +
                        (request.topic.empty() ? "bai hoc" : request.topic) + "\"?";
        draft.options = {
            "Phuong an dung theo kien thuc bai hoc",
            "Phuong an gan dung nhung thieu y quan trong",
            "Phuong an khong lien quan den chu de",
            "Phuong an trai voi noi dung bai hoc"
        };
...
bool AIQuestionService::callRealApi(const AIQuestionRequest& request,
    ...
    if (apiKey.empty()) {
        lastError = "Chua cau hinh AI that. Dat GROQ_API_KEY hoac QUIZAPP_AI_API_KEY, roi dat QUIZAPP_AI_MODE=real.";
        return false;
    }
    ...
    if (curl == nullptr) {
        lastError = "Khong khoi tao duoc ket noi AI.";
        return false;
    }
...
bool AIQuestionService::parseQuestionsObject(const json& root,
    ...
    for (const json& item : root.at("questions")) {
        ...
        if (!validateDraft(draft, validationError)) {
            errorMessage = "Cau hoi AI khong hop le: " + validationError;
            return false;
        }
        questions.push_back(draft);
    }
